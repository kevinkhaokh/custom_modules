...
class reconcile_wizard_temp_screen(models.TransientModel):

    _name = "dih.temp.reconcile.report.temp.screen.wizard"
    _description = "Temporary Reconciles Wizard Confirmation Screen"

    @api.multi
    def temp_report_reconciles(self):
        ...
        acr = self.env.cr
        auid = self.env.uid
        return self.pool.get('report').get_action(acr, auid, all_account_moves_ids, 'dih_accounting_manual_reconcile_reports.report_dih_temp_reconcile')
#        return {
